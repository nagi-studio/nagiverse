...
纯内容型 PR 会额外走一层自动内容审核：

- 如果只是知识分享内容，且没有明显事实风险、广告导流、政治宣传或恶意内容，系统会自动启用 GitHub auto-merge。
- 如果缺少用户资料、来源、正文过短，或还有其他注意点，系统会在 PR 里直接提醒，暂不自动合并。
...
- **排版规范。** 中英文、数字之间需要加空格（例如「使用 Rust 编写」而不是「使用Rust编写」）。CI 会自动修正，但建议本地安装 [AutoCorrect](https://github.com/huacnlee/autocorrect) 插件，写的时候就能即时格式化。
